...
export interface CardProps {
  ...
  html_url: string;
  languages?: string[];
  habilidades?: string[]; // Adicionando a propriedade 'habilidades'
}

export const Card = ({ name, description, html_url, languages, habilidades }: CardProps) => {
  return (
      ...
      <div className="bg-primary-blue w-[300px] rounded-[8px] flex flex-col items-center justify-between p-5 text-center shadow-md shadow-light-blue">
        ...
        <div className="flex items-center mt-4">
          <a href={html_url} target="_blank" rel="noreferrer noopener" className="flex items-center gap-x-3 p-1 rounded-[6px] hover:bg-light-coral hover:text-primary-blue transition-all ease-in-out duration-500 text-med-gray">
            Exibir credencial: <FaAward className="w-8 h-8 cursor-pointer rounded-[4px]" />
          </a>
        </div>
